...
```
srun -n 2 python xirun_folder.py --nthreads 64 ...
```
...
from kcorr.k_corrections import selection
...
logger = logging.getLogger('xirun_folder')
...
def corr_fn(file_type='npy', region='', tracer='ELG', tracer2=None, recon_dir='n',rec_type=False, weight_type='default', bin_type='lin', njack=0, nradjack=0, nrandoms=8, split_randoms_above=10, out_dir='.', wang=None, rpcut=None, thetacut=None):
    ...
    root = '{}{}_{}_{}_njack{:d}{}_nran{:d}{}'.format(wang, tracer, weight_type, bin_type, njack, radjack, nrandoms, split)
...
if __name__ == '__main__':
    ...
    logger.info('Checking for clustering catalogs in {} for tracer {}.'.format(cat_dir, tracer))

    # Now search the cat_dir for catalogs and process all of them 
    # names like BGS_BRIGHT_CEN_mag-22.6473to-21.9413_gr0.9778to1.0755_NGC_clustering.dat.fits
    # names like {TRACER}_{SUBSAMPLE}_{REG}_clustering.dat.fits
    # randoms like BGS_BRIGHT_CEN_mag-20.7092to-20.3206_gr0.7922to0.9222_NGC_0_clustering.ran.fits
    # randoms like {TRACER}_{SUBSAMPLE}_{REG}_{NUM}_clustering.ran.fits
    folder_path = Path(cat_dir)
    files = [f for f in folder_path.iterdir() if f.suffix == ".fits"]
    ...
    for f in files:
        ...
        tracer_full = f"{tracer}_{subsample}"
        if tracer_full in processed:
            continue
        processed.append(tracer_full)

        catalog_kwargs = dict(tracer=tracer_full, tracer2=tracer2, survey=args.survey, cat_dir=cat_dir, recon_dir=args.recon_dir,rec_type=args.rec_type) # survey required for zdone
        base_file_kwargs = dict(tracer=tracer_full, tracer2=tracer2, recon_dir=args.recon_dir,rec_type=args.rec_type, weight_type=args.weight_type, bin_type=bintype_str, njack=args.njack, nradjack=args.nradjack, nrandoms=args.nran, split_randoms_above=args.split_ran_above, rpcut=args.rpcut, thetacut=args.thetacut)
