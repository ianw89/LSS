...
parser.add_argument("--magbins", help="number of adaptive bins to use for the mag binning (first-level split)", type=int, default=10)
parser.add_argument("--splitprop", help="name of the column to use for the adaptive sub-binning done within each mag bin x quiescent/star-forming split", type=str, default=None)
...
def process_sample(tracer_out, fout, dat_inallbins, zmin, zmax, zmax_arr):
    if args.mkfulldat == 'y':
    ...
    if mkclusran:
        ...
        def _parfun_cr(ii):
            # mkclusran mutates the Table it's given in-place (drops columns like TILEID via keep_columns),
            # so pass a copy to avoid corrupting the shared rand_tbls entry across mag/prop bins
            ranin = rand_tbls[ii].copy()
    ...
    if args.imsys_clus == 'y':
        ...
        if not skip_imsys:
            ...
            for iran in range(0,args.nran4imsys):
                ...
                ranl.append(ran)
            # Random files from a partially-completed previous run may have extra columns (e.g. NX, WEIGHT_FKP
            # added by a subsequent nz step) that others lack; restrict to columns common to all before concatenating
            common_cols = set(ranl[0].dtype.names)
            for ran in ranl[1:]:
                common_cols &= set(ran.dtype.names)
            common_cols = [c for c in ranl[0].dtype.names if c in common_cols]
            if any(set(ran.dtype.names) != set(common_cols) for ran in ranl):
                common.printlog(f'Random files have mismatched columns, restricting to common columns: {common_cols}', logger)
            ranl = [ran[common_cols] for ran in ranl]
...
if args.splitprop is not None and args.splitprop not in fulldat.colnames:
...
mag_bins = common.make_adaptive_density_bins(fulldat['ABS_MAG_R'], n_bins=args.magbins, n_tail=0, alpha=0.5, limit=0.01)
...
np.save(args.outdir+'/'+args.input_tracer+'_mag_bins'+str(args.magbins)+'.npy', mag_bins)
np.save(args.outdir+'/'+args.input_tracer+'_mag_bins'+str(args.magbins)+'_means.npy', mag_bins_means)
    ...
    for is_quiescent in [True, False]:
        ...
        if args.splitprop is not None:
            ...
            for j in range(len(prop_bins)-1):
                ...
                process_sample(tracer_out, fout, dat_inallbins, zmin, zmax, zmax_arr)

        else:
            common.printlog(f'Mag bin {i}: {mag_bins[i]:.4f} to {mag_bins[i+1]:.4f}, ({qsf_label}) count: {len(dat_inqbin)}', logger)

            #write output to new "full" catalog at your defined location
            tracer_out = args.input_tracer + "_CEN" + f'_mag{mag_bins[i]:.2f}to{mag_bins[i+1]:.2f}' + f'_{qsf_label}'
            fout = args.outdir+'/'+tracer_out+'_full'+args.use_map_veto+'.dat.fits'

            zmax_arr = np.where(dat_inqbin['PHOTSYS'] == b'N', zmax_N, zmax_S)

            process_sample(tracer_out, fout, dat_inqbin, zmin, zmax, zmax_arr)
